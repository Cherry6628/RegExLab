...
Rules:
    Rules for XSS:
        ...
        Sometimes, there may be no fix needed, then leave it as it is, and dont add encoding methods.  For example, querySelector(xyz) should remain querySelector(xyz) and no need for encoding.  querySelector(), innerText = , textContent = , for cases like these, there is no need for encoding
