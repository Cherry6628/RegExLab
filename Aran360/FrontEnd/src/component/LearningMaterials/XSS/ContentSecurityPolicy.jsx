    ...
    return(
        ...
        <section className="mainbar">
            <section>
                ...
                <p>To enable CSP, a response needs to include an HTTP response header called <span>Content-Security-Policy</span> with a value containing the policy. The policy itself consists of one or more directives, separated by semicolons.</p>
            </section>
        <section>
            ...
            <p>It's quite common for a CSP to block resources like <span>script</span>. However, many CSPs do allow image requests. This means you can often use <span>img</span> elements to make requests to external servers in order to disclose CSRF tokens, for example.</p>
        ...
        <section>
            ...
            <Payloads>img-src https://images.normal-website.com</Payloads>
            <p>Note that these policies will prevent some dangling markup exploits, because an easy way to capture data with no user interaction is using an <span>img</span> tag. However, it will not prevent other exploits, such as those that inject an anchor tag with a dangling <span>href</span> attribute.</p>
        </section>
        <section>
