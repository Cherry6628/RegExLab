    ...
    return(
        ...
        <section className="mainbar">
            <section>
                ...
                <p>In this section, we'll describe some general principles for preventing cross-site scripting vulnerabilities and ways of using various common technologies for protecting against XSS attacks.</p>
                <p>Cross-site scripting prevention can generally be achieved via two layers of defense:</p>
                <ul>
                    <li>Encode data on output</li>
                    <li>Validate input on arrival</li>
                </ul>
                <p>You can use Burp Scanner to scan your web sites for numerous security vulnerabilities including XSS. Burp's cutting-edge scanning logic replicates the actions of a skilled attacker and is able to achieve correspondingly high coverage of XSS vulnerabilities. You can use Burp Scanner to gain assurance that your defenses against XSS attacks are working effectively.</p>
                <p>Learn more about Burp Scanner</p>
            </section>
            <section>
                <h1>Encode data on output</h1>
                <p>Encoding should be applied directly before user-controllable data is written to a page, because the context you're writing into determines what kind of encoding you need to use. For example, values inside a JavaScript string require a different type of escaping to those in an HTML context.</p>
                <p>In an HTML context, you should convert non-whitelisted values into HTML entities:</p>
                <ul>
                    <li>{'<'} converts to: {'&lt;'}</li>
                    <li>{'>'} converts to: {'&gt;'}</li>
                </ul>
                <p>In a JavaScript string context, non-alphanumeric values should be Unicode-escaped:</p>
                <ul>
                    <li>{'<'} converts to: {'\u003c'}</li>
                    <li>{'>'} converts to: {'\u003e'}</li>
                </ul>
                ...
                <section>
                    ...
                    <p>Encoding is probably the most important line of XSS defense, but it is not sufficient to prevent XSS vulnerabilities in every context. You should also validate input as strictly as possible at the point when it is first received from a user.</p>
                    <p>Examples of input validation include:</p>
                    <ul>
                        <li>If a user submits a URL that will be returned in responses, validating that it starts with a safe protocol such as HTTP and HTTPS. Otherwise someone might exploit your site with a harmful protocol like javascript or data.</li>
                        <li>If a user supplies a value that it expected to be numeric, validating that the value actually contains an integer.</li>
                        <li>Validating that input contains only an expected set of characters.</li>
                    </ul>
                    <p>Input validation should ideally work by blocking invalid input. An alternative approach, of attempting to clean invalid input to make it valid, is more error prone and should be avoided wherever possible.</p>
                    <h3>Whitelisting vs blacklisting</h3>
                    <p>Input validation should generally employ whitelists rather than blacklists. For example, instead of trying to make a list of all harmful protocols (javascript, data, etc.), simply make a list of safe protocols (HTTP, HTTPS) and disallow anything not on the list. This will ensure your defense doesn't break when new harmful protocols appear and make it less susceptible to attacks that seek to obfuscate invalid values to evade a blacklist.</p>
                </section>
                <section>
            ...
            <section>
                ...
                <p>In PHP there is a built-in function to encode entities called htmlentities. You should call this function to escape your input when inside an HTML context. The function should be called with three arguments:</p>
                <ul>
                    <li>Your input string.</li>
                    <li>ENT_QUOTES, which is a flag that specifies all quotes should be encoded.</li>
